...
class _DetectLAMMPSbond(_Detect):
    ...
    def _readstepfunc(self, item) -> Tuple[List[bytes], Tuple[int, int]]:
        ...
        bond = [b if b is not None else [] for b in bond]
        level = [l if l is not None else [] for l in level]
        bond, level = self._filter_PtPt_bonds(bond, level)
...
class _DetectCrd(_Detect):
    def _getbondfromcrd(self, step_atoms: Atoms, cell: np.ndarray) -> Tuple[List[List[int]], List[List[int]]]:
        ...
        orig_bond = [[] for _ in range(atomnumber)]
        orig_level = [[] for _ in range(atomnumber)]
        for b in openbabel.OBMolBondIter(mol):
            ...
            if level == 5:
                level = 12  
            orig_bond[s1].append(s2)
            orig_bond[s2].append(s1)
            orig_level[s1].append(level)
            orig_level[s2].append(level)

        symbols = step_atoms.get_chemical_symbols()
        pos = step_atoms.positions
        N = atomnumber
        ...
        pair_level = {}
        for i in range(N):
            for idx, j in enumerate(orig_bond[i]):
                if i < j:
                    orig_pairs.add((i, j))
                    pair_level[(i, j)] = orig_level[i][idx]

        is_pt = [s == 'Pt' for s in symbols]
        is_o  = [s == 'O' for s in symbols]
        is_h  = [s == 'H' for s in symbols]

        should_exist = set()
        for i, j in orig_pairs:
            if is_pt[i] and is_pt[j]:
                continue   
            should_exist.add((i, j))

        pt_indices = [i for i, flag in enumerate(is_pt) if flag]
        o_indices  = [j for j, flag in enumerate(is_o) if flag]
        for i in pt_indices:
            for j in o_indices:
                if i == j:
                    continue
                if i > j:
                    i, j = j, i   
                if self.pbc:
                    dist = step_atoms.get_distance(i, j, mic=True)
                else:
                    dist = np.linalg.norm(pos[i] - pos[j])
                if dist < 2.8:
                    should_exist.add((i, j))
                else:
                    should_exist.discard((i, j))

        for i in o_indices:
            for j, flag_h in enumerate(is_h):
                if not flag_h:
                    continue
                if i == j:
                    continue
        ...
        final_bond = [[] for _ in range(N)]
        final_level = [[] for _ in range(N)]
        for i, j in should_exist:
            level = pair_level.get((i, j), 1)
            final_bond[i].append(j)
            final_bond[j].append(i)
            final_level[i].append(level)
            final_level[j].append(level)

        return final_bond, final_level
...
class _DetectLAMMPSdump(_DetectCrd):
    ...
    def _filter_PtPt_bonds(self, bond, level):
        N = self.N
        bonds_to_keep = []
        for i in range(N):
            bi = bond[i] if bond[i] is not None else []
            li = level[i] if level[i] is not None else []
            for j, l in zip(bi, li):
                if i < j:
                    atom_i = self.atomname[self.atomtype[i]]
                    atom_j = self.atomname[self.atomtype[j]]
                    if atom_i == 'Pt' and atom_j == 'Pt':
                        continue  
                    bonds_to_keep.append((i, j, l))
    
        new_bond = [[] for _ in range(N)]
        new_level = [[] for _ in range(N)]
        for i, j, l in bonds_to_keep:
            new_bond[i].append(j)
            new_bond[j].append(i)
            new_level[i].append(l)
            new_level[j].append(l)
        return new_bond, new_level
